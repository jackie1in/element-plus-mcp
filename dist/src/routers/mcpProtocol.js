 ...
 * 用于在MCP协议外直接调用生成组件功能
 */
router.post("/execute-generate-tool", async (ctx) => {
    try {
        const { description, componentType, stylePreference, featuresRequired, llmConfig, } = ctx.request.body;
        // 验证必要的输入参数
        if (!description) {
            ctx.status = 400;
            ctx.body = {
 ...
 * 使用提示模板生成组件
 */
router.post("/use-prompt-template", async (ctx) => {
    try {
        const { templateName, variables, llmConfig } = ctx.request.body;
        // 验证必要的输入参数
